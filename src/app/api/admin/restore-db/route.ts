...
// backup file. Photos/attachments in most tables live inline as base64 rows
// (VehicleFile/TuneLog store binaries on disk, not in this DB — see their API
// routes — so a full self-hosted migration should also carry over the
// per-vehicle upload directory separately; this route only handles the SQLite
// database itself). A single request here replaces the ENTIRE database, so
// the ambient session cookie alone isn't enough — the password must be
// re-entered in the request.
